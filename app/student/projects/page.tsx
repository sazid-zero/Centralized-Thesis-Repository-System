...
export default function StudentProjectsPage() {
    ...
    const stats = [
        ...
        { label: "In Progress", value: projects.filter(p => p.status === "in_progress").length.toString(), icon: Clock, color: "text-blue-600" },
        { label: "Published", value: projects.filter(p => p.status === "published").length.toString(), icon: CheckCircle, color: "text-green-600" },
                                    ...
                                    <Link key={project.id} href={`/student/projects/${project.id}`}>
                                        <Card className="border-border bg-card p-4 sm:p-6 hover:shadow-lg transition-shadow cursor-pointer h-full">
                                            <div className="flex items-start justify-between gap-3 mb-3">
                                                ...
                                                <Badge className={`${getStatusColor(project.status)} text-xs flex items-center gap-1 flex-shrink-0`}>
                                            ...
                                            <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    <span>{project.collaborators.filter(c => c.invitationStatus === 'accepted').length} members</span>
                                                </div>
